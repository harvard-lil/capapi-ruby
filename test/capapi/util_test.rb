  ...
  class UtilTest < Test::Unit::TestCase
    context "OPTS_COPYABLE" do
      should "include :apibase" do
        assert_include Capapi::Util::OPTS_COPYABLE, :api_base
      end
    end

    context "OPTS_PERSISTABLE" do
      should "include :client" do
        assert_include Capapi::Util::OPTS_PERSISTABLE, :client
      end

      should "not include :idempotency_key" do
        refute_includes Capapi::Util::OPTS_PERSISTABLE, :idempotency_key
      end
    end
    ...
    should "#encode_parameters should prepare parameters for an HTTP request" do
      ...
      assert_equal(
        ...
        Capapi::Util.encode_parameters(params)
      )
    end

    should "#url_encode should prepare strings for HTTP" do
      assert_equal "foo",      Capapi::Util.url_encode("foo")
      assert_equal "foo",      Capapi::Util.url_encode(:foo)
      assert_equal "foo%2B",   Capapi::Util.url_encode("foo+")
      assert_equal "foo%26",   Capapi::Util.url_encode("foo&")
      assert_equal "foo[bar]", Capapi::Util.url_encode("foo[bar]")
    end
    ...
    should "#flatten_params should encode parameters according to Rails convention" do
      ...
      assert_equal([
        ...
        ["f[1][foo]", "3"],
        ["f[1][bar]", "4"],
      ], Capapi::Util.flatten_params(params))
    end
    ...
    should "#symbolize_names should convert names to symbols" do
      ...
      symbolized = Capapi::Util.symbolize_names(start)
      assert_equal(finish, symbolized)
    end

    should "#normalize_opts should reject nil keys" do
      assert_raise { Capapi::Util.normalize_opts(nil) }
    end

    should "#convert_to_capapi_object should pass through unknown types" do
      obj = Util.convert_to_capapi_object(7, {})
      assert_equal 7, obj
    end

    should "#convert_to_capapi_object should turn hashes into CapapiObjects" do
      obj = Util.convert_to_capapi_object({ foo: "bar" }, {})
      assert obj.is_a?(CapapiObject)
      assert_equal "bar", obj.foo
    end

    should "#convert_to_capapi_object should turn lists into ListObjects" do
      obj = Util.convert_to_capapi_object({ object: "list" }, {})
      assert obj.is_a?(ListObject)
    end

    should "#convert_to_capapi_object should marshal other classes" do
      obj = Util.convert_to_capapi_object({ object: "court" }, {})
      assert obj.is_a?(Court)
    end

    should "#convert_to_capapi_object should marshal arrays" do
      obj = Util.convert_to_capapi_object([1, 2, 3], {})
      assert_equal [1, 2, 3], obj
    end

    context ".log_*" do
      setup do
        @old_log_level = Capapi.log_level
        Capapi.log_level = nil
      ...
      teardown do
        Capapi.log_level = @old_log_level
        $stderr = @old_stderr
        $stdout = @old_stdout
      end
    ...
    context ".log_internal" do
      should "log in a terminal friendly way" do
        ...
        Util.send(:log_internal, "message", { foo: "bar" },
                  color: :green, level: Capapi::LEVEL_DEBUG, logger: nil, out: out)
      ...
      should "log to a logger if set" do
        ...
        Util.send(:log_internal, "message", { foo: "bar" },
                  color: :green, level: Capapi::LEVEL_DEBUG, logger: logger, out: $stdout)
